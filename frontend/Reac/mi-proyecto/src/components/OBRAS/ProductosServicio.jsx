...
const ProductosServicio = ({ notificacion, onToast, showHeader = true, onFinalizarEntrega }) => {
  ...
  const [cortesVidrioAgregados, setCortesVidrioAgregados] = useState([]);
  const [corteVidrioInput, setCorteVidrioInput] = useState({ ancho: '', alto: '', cantidad: '1' });
  const [cortesAluminioAgregados, setCortesAluminioAgregados] = useState([]);
  const [corteAluminioInput, setCorteAluminioInput] = useState({ largo: '', cantidad: '1' });
  ...
  const cortesAluminioActivos = cortesAluminioAgregados.length > 0 ? cortesAluminioAgregados : cortesAluminio;
  const totalUsadoCm = cortesAluminioActivos.reduce((acc, corte) => acc + (corte.largo_cm || 0), 0);
  ...
  const calcularDistribucionVidrio = (cortesADistr = cortesVidrioAgregados.length > 0 ? cortesVidrioAgregados : cortesVidrioSeleccionado) => {
  ...
  const handleAgregarCorteVidrio = () => {
    const ancho = parseFloat(corteVidrioInput.ancho);
    const alto = parseFloat(corteVidrioInput.alto);
    const cantidad = parseInt(corteVidrioInput.cantidad) || 1;
    
    if (ancho && alto && !isNaN(ancho) && !isNaN(alto) && ancho > 0 && alto > 0 && cantidad > 0) {
      const nuevosCortes = [];
      for (let i = 0; i < cantidad; i++) {
        nuevosCortes.push({ id: Date.now() + i, ancho_cm: ancho, alto_cm: alto });
      }
      setCortesVidrioAgregados([...cortesVidrioAgregados, ...nuevosCortes]);
      setCorteVidrioInput({ ancho: '', alto: '', cantidad: '1' });
    } else {
  ...
  const handleEliminarCorteVidrio = (id) => {
    setCortesVidrioAgregados(cortesVidrioAgregados.filter(c => c.id !== id));
  };

  const handleAgregarCorteAluminio = () => {
    const largo = parseFloat(corteAluminioInput.largo);
    const cantidad = parseInt(corteAluminioInput.cantidad) || 1;
    
    if (largo && !isNaN(largo) && largo > 0 && cantidad > 0) {
      const nuevosCortes = [];
      for (let i = 0; i < cantidad; i++) {
        nuevosCortes.push({ id: Date.now() + i, largo_cm: largo });
      }
      setCortesAluminioAgregados([...cortesAluminioAgregados, ...nuevosCortes]);
      setCorteAluminioInput({ largo: '', cantidad: '1' });
    } else {
      onToast && onToast('Ingresa valores válidos para largo y cantidad', 'error');
    }
  };

  const handleEliminarCorteAluminio = (id) => {
    setCortesAluminioAgregados(cortesAluminioAgregados.filter(c => c.id !== id));
  };
  ...
  const guardarProgresoProductos = async () => {
    ...
    try {
      ...
      const datosCortes = {
        cortes_vidrio: cortesVidrioAgregados.length > 0 ? cortesVidrioAgregados : [],
        cortes_aluminio: cortesAluminioAgregados.length > 0 ? cortesAluminioAgregados : [],
        total_cortes: (cortesVidrioAgregados.length || 0) + (cortesAluminioAgregados.length || 0),
        productos_utilizados: productosSeleccionados?.length || 0
      };

      // Obtener personalToken del localStorage
      const personalToken = localStorage.getItem('personalToken');
      const personalId = localStorage.getItem('personal_id');

      if (!personalId) {
        onToast && onToast('Error: No se encontró el ID del personal', 'error');
        setFinalizando(false);
        return;
      }

      const response = await fetch('/api/progreso/guardar-productos', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        onToast && onToast('✅ Progreso de productos guardado (75% completado)', 'success');
        console.log('Progreso guardado:', data);
      } else {
        onToast && onToast(data.message || 'Error al guardar progreso', 'error');
        console.error('Error en respuesta:', data);
      }
    } catch (error) {
      console.error('Error guardando progreso:', error);
      onToast && onToast('Error al guardar progreso: ' + error.message, 'error');
    } finally {
      setFinalizando(false);
    }
  };
  ...
  const manejarGuardar = async () => {
    ...
    await guardarProgresoProductos();
    // Luego finalizar la entrega
    await finalizarEntregaCompleta();
  };
        ...
        <div style={{ border: `2px solid ${COLORS.text}`, borderRadius: '8px', padding: '12px' }}>
          ...
          {barras.map((barra) => (
            <div key={barra.id} style={{ border: `1px solid ${COLORS.text}`, borderRadius: '4px', padding: '10px', marginBottom: '12px', backgroundColor: COLORS.backgroundLight }}>
              ...
              {/* Medidas de Aluminio - SIEMPRE CON MÚLTIPLES CORTES */}
              <div style={{ marginTop: '12px' }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, marginBottom: 4, fontFamily: FONTS.heading, color: COLORS.text }}>AGREGAR CORTE (cm)</label>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                  <input
                    type="number"
                    placeholder="Largo (cm)"
                    value={corteAluminioInput.largo}
                    onChange={(e) => setCorteAluminioInput({ ...corteAluminioInput, largo: e.target.value })}
                    style={{
                      flex: 1,
                      padding: '6px',
                  ...
                  <input
                    ...
                    value={corteAluminioInput.cantidad}
                    onChange={(e) => setCorteAluminioInput({ ...corteAluminioInput, cantidad: e.target.value })}
                    style={{
                      width: '80px',
                      padding: '6px',
                      border: `1px solid ${COLORS.text}`,
                  ...
                  />
                  <button
                    onClick={handleAgregarCorteAluminio}
                    style={{
                      padding: '6px 12px',
                ...
                {cortesAluminioAgregados.length > 0 && (
                  <div style={{ background: COLORS.gray[100], border: `1px solid ${COLORS.border}`, borderRadius: '4px', padding: '8px', marginBottom: '8px' }}>
                    <div style={{ fontSize: 10, fontWeight: 600, marginBottom: '6px', fontFamily: FONTS.heading, color: COLORS.text }}>Cortes agregados ({cortesAluminioAgregados.length}):</div>
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                      {cortesAluminioAgregados.map((corte, idx) => (
                        <div key={corte.id} style={{ 
                          ...
                          <button
                            onClick={() => handleEliminarCorteAluminio(corte.id)}
                            style={{
                              background: 'transparent',
                              border: 'none',
                              color: COLORS.error,
              ...
              <div style={{ marginTop: '12px' }}>
                ...
                {cortesAluminio.length > 0 || cortesAluminioAgregados.length > 0 ? (
                  <svg 
                    ref={svgAluminioRef}
                    width="100%" 
                    height="50" 
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const cortesAMostrar = cortesAluminioAgregados.length > 0 ? cortesAluminioAgregados : cortesAluminio;
                      return cortesAMostrar.map((corte, idx) => {
        ...
        <div>
          <div style={{ border: `2px solid ${COLORS.text}`, borderRadius: '4px', padding: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                <input
                  type="number"
                  placeholder="Ancho"
                  value={corteVidrioInput.ancho}
                  onChange={(e) => setCorteVidrioInput({ ...corteVidrioInput, ancho: e.target.value })}
                  style={{
                    flex: 1,
                    padding: '6px',
                    border: `1px solid ${COLORS.text}`,
                ...
                <input
                  type="number"
                  placeholder="Alto"
                  value={corteVidrioInput.alto}
                  onChange={(e) => setCorteVidrioInput({ ...corteVidrioInput, alto: e.target.value })}
                  style={{
                    flex: 1,
                    padding: '6px',
                    border: `1px solid ${COLORS.text}`,
                ...
                <input
                  type="number"
                  placeholder="Cantidad"
                  value={corteVidrioInput.cantidad}
                  onChange={(e) => setCorteVidrioInput({ ...corteVidrioInput, cantidad: e.target.value })}
                  style={{
                    width: '80px',
                    padding: '6px',
                    border: `1px solid ${COLORS.text}`,
              ...
              {/* Lista de cortes agregados */}
              {cortesVidrioAgregados.length > 0 && (
                <div style={{ background: COLORS.gray[100], border: `1px solid ${COLORS.border}`, borderRadius: '4px', padding: '8px' }}>
                  <div style={{ fontSize: 10, fontWeight: 600, marginBottom: '6px', fontFamily: FONTS.heading, color: COLORS.text }}>Cortes agregados ({cortesVidrioAgregados.length}):</div>
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                    {cortesVidrioAgregados.map((corte, idx) => (
                      <div key={corte.id} style={{ 
                        background: COLORS.white, 
                        border: `1px solid ${COLORS.text}`, 
                        borderRadius: '3px', 
            ...
            <div style={{
              ...
              {cortesVidrioSeleccionado.length === 0 && cortesVidrioAgregados.length === 0 ? (
